...
description: Ask user if they would like to provide feedback. Call scripts/mlflow_feedback.py. Don't ask any follow up questions.
allowed-tools:
  - write
  - read_file
---
...
Step 4 [Claude] You MUST run scripts/mlflow_feedback.py \
  --category {Category} \
  --skill {Skills} \
...
3. You MUST use scripts/mlflow_feedback.py

## Feedback format
Take the comment the user gave as feedback and create the following inputs for scripts/mlflow_feedback.py and you must call scripts/mlflow_feedback.py
...
Run: python scripts/mlflow_feedback.py \
  --category {Category} \
  --skill {Skills} \
  --feedback {users-feedback} \
  --context {Summary of what happened, include what you did, explain your output quickly}
