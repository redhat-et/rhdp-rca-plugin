...
if __name__ == "__main__" and __package__ is None:
    ...
    from scripts.parse_github_paths import parse_github_paths
else:
    ...
    from .parse_github_paths import parse_github_paths
...
def save_step(analysis_dir: Path, step: int | str, data: dict) -> Path:
...
def get_step_name(step: int | str) -> str:
...
def cmd_analyze(args: argparse.Namespace, config: Config) -> int:
    ...
    print("\n[Step 4a] Parsing GitHub paths...")
    try:
        github_paths = parse_github_paths(job_context)
        step4a_path = save_step(analysis_dir, "4a", github_paths)

        print(f"  Parsing status: {github_paths.get('parsing_status')}")
        if github_paths.get("warnings"):
            for w in github_paths["warnings"]:
                print(f"    Warning: {w}")
        print(f"  Failed tasks: {len(github_paths.get('failed_tasks', []))}")
        print(
            f"  Total files to fetch: {github_paths.get('fetch_instructions', {}).get('total_files')}"
        )
        print(f"  Output: {step4a_path}")
    except Exception as e:
        print(f"  Error parsing GitHub paths: {e}")
        # Don't fail the entire analysis - Claude can still work with steps 1-3
    ...
    print("  - step4a_github_paths.json")

    print("\n[Step 4b-4e] Next: Fetch GitHub files and analyze")
    print("  Claude will fetch config/workload files and create step4_summary.json")
    print("  See SKILL.md for analysis instructions")
